-- read-only CI checks view (§8.2): :Dipher pr checks lists the status-check rollup and
...
local STYLE = {
    success = { hl = "dipherPanelAdd", glyph = "✓" },
    failure = { hl = "dipherPanelDelete", glyph = "✗" },
    pending = { hl = "dipherPanelModify", glyph = "●" },
    neutral = { hl = "dipherPanelHelp", glyph = "○" },
}
...
local function render(checks)
    ...
    local ns = vim.api.nvim_create_namespace("dipher.checks")
...
-- :Dipher pr checks — fetch the rollup for the live session's PR and show the float
