...
local ui = require("dipher.ui.overview")

local M = {}

local ns
local function namespace()
    ns = ns or vim.api.nvim_create_namespace("dipher.pr.overview")
    return ns
end

local GUARD = "dipher.pr.overview.guard"
...
local function still_live(session)
    return require("dipher.pr").current_session() == session
end
...
local function arm_guard(session, win)
    ...
    vim.api.nvim_create_autocmd("WinClosed", {
        ...
        callback = function()
            local s = require("dipher.pr").current_session()
            if s == session and not s.panel then
                require("dipher.pr").end_session()
            end
        end,
    })
end
...
local function set_keymaps(b)
    local function live()
        return require("dipher.pr").current_session()
    end
    local function open_url()
        ...
        if url and url ~= "" then
            ...
            require("dipher.pr").notify("no PR url", vim.log.levels.WARN)
        end
    end
    local function enter(review)
        ...
        if review then
            require("dipher.pr").review({ number = s.pr.number })
        else
            require("dipher.pr").view({ number = s.pr.number })
        end
    end
...
local function paint(built)
    if not (buf and vim.api.nvim_buf_is_valid(buf)) then
        ...
        pcall(vim.api.nvim_buf_set_name, buf, "dipher://overview")
        set_keymaps(buf)
    end
...
-- review (:Dipher pr overview) — the content window, closing the diff + hiding the panel
...
function M.open(session)
    if not session then
        return require("dipher.pr").notify("open a PR first")
    end
